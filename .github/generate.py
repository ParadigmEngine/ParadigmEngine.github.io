...
def run_command(directory=None, command=[], verbose=False):
    if verbose:
        print(f"executing '{' '.join(os.fspath(comm) if isinstance(comm, pathlib.Path) else comm for comm in command)}' in '{directory or CURRENT_DIR}'")
    output_lines = subprocess.Popen(command, stdout=subprocess.PIPE, cwd=directory, stderr=subprocess.DEVNULL).stdout.readlines()
        ...
        if not regenerate and info_file is not None and pathlib.Path.exists(info_file):
            ...
            with pathlib.Path.open(info_file.resolve(), 'rb') as file:
                ...
                if self.version == GENERATOR_VERSION:
                    ...
                    raise Exception("Unhandled version, the cache contains version '{self.version}' while the generator is '{GENERATOR_VERSION}', and no migration was supported.")

    def save(self) -> None:
        with open(self.__file, 'w') as file:
            data = {'version': GENERATOR_VERSION, 'tags': self.tags, 'branches': self.branches, 'generator': self.generator}
...
class Repository:
    def __init__(self, url, verbose=False) -> None:
        ...
        self.tags     = _git_entry_to_dict([(tag[:SHA1_LENGTH], tag[SHA1_LENGTH:]) for tag in run_command(command=['git', 'ls-remote', '--tags', url], verbose=self.verbose)], 'refs/tags/')
        self.branches = _git_entry_to_dict([(branch[:SHA1_LENGTH], branch[SHA1_LENGTH:]) for branch in run_command(command=['git', 'ls-remote', '--heads', url], verbose=self.verbose)], 'refs/heads/')
    ...
    def checkout(self, branch) -> None:
        ...
        run_command(path, ['git', 'clone', '--quiet', '--depth', '1', self.url, '--branch', branch, '--single-branch', '.'], self.verbose)
        return path
...
def generate(repository, generator, info=Info(), verbose=False):
    ...
    generator_sha1 = [line[:SHA1_LENGTH] for line in run_command(command=['git', 'ls-remote', generator, 'HEAD'], verbose=verbose)]
    ...
    if info.generator == generator_sha1:
        ...
        # prune pre-existing
        [ _remove_directory(pathlib.Path.joinpath(ROOT_DIR, tag), include_self=True) for tag in info.tags if tag not in repository.tags]
        [ _remove_directory(pathlib.Path.joinpath(ROOT_DIR, branch), include_self=True) for branch in info.branches if branch not in repository.branches]
        
        tags = {tag: repository.tags[tag] for tag in repository.tags if (tag not in info.tags or info.tags[tag] != repository.tags[tag])}
        branches = {branch: repository.branches[branch] for branch in repository.branches if (branch not in info.branches or info.branches[branch] != repository.branches[branch])}
    elif verbose:
        print(f"generator was updated [old:'{info.generator}' new:'{generator_sha1}'], regenerating all")
    ...
    def generate_for(generator_dir, repository, name, sha1, verbose=False):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            run_command(directory=css_path, command=[PYTHON_COMMAND, 'postprocess.py', 'm-dark.css', 'm-documentation.css', '-o', 'm-dark+documentation.compiled.css'], verbose=verbose)
            run_command(directory=documentation_path, command=[PYTHON_COMMAND, 'doxygen.py', doxyfile.resolve(), '--output-dir', temp_dir], verbose=verbose)
    ...
    if len(tags) != 0 or len(branches) != 0:
        ...
        with tempfile.TemporaryDirectory() as generator_root:
            ...
            run_command(directory=generator_root, command=['git', 'clone', generator, '--depth', '1', '.'], verbose=verbose)
...
if __name__ == "__main__":
    parser = ArgumentParser(description='Conditionally generate, or update the documentation based on new tags, or generator changes.', formatter_class=ArgumentDefaultsHelpFormatter)
